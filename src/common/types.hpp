#ifndef SRC_COMMON_TYPES_HPP
#define SRC_COMMON_TYPES_HPP

#include <cstdint>

typedef float autoraufwand_t;
typedef uint_fast32_t autorid_t;
typedef uint_fast8_t breitengrad_t;
typedef uint_fast32_t ereignis_nr_t;
typedef float ereignis_wert_t;
typedef uint_fast32_t fahrstr_register_nr_t;
typedef float geschwindigkeit_t;
typedef float helligkeit_t;
typedef float kilometrierung_t;
typedef float koordinate_t;
typedef float kruemmung_t;
typedef uint_fast32_t objektid_t;
typedef float meter_t;
typedef uint_fast8_t nachfolger_index_t;
typedef float ueberhoehung_t;
typedef uint_fast32_t referenz_nr_t;
typedef uint_fast8_t referenz_typ_t;
typedef uint_fast32_t rekursionstiefe_t;
typedef float spannung_t;
typedef uint_fast32_t streckenelement_nr_t;
typedef bool streckenelement_richtung_t;

#endif
